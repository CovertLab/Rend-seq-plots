...
for i = 1:(n_ends-1)
    ...
    % ignore if most reads for isoform are 0
    if sum(data_3(av_range) > 0) / length(data_3(av_range)) < 0.20
        levels_subregions(i) = 0;
    end
...
if isnan(levels_subregions)
    levels_subregions(:) = 0;
    lengths_subregions(:) = 0;
    return
end
