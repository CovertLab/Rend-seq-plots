function [x_3, x_5, isoform_levels] = quantify_plot_isoforms_20180130(start_region,stop_region,...
    ...
    spurious_5,spurious_3,genome_size,annotation_file,annotation_dir,index)
...
if length(x_rec) <= 1
	x_rec = start_region:stop_region;
	isoform_levels = zeros(length(x_rec), 1);
	rec_trace = zeros(length(x_rec), 1);
end
...
% abundance.
if ~strand_region
    temp = genome_size - stop_region + 1;
    stop_region = genome_size - start_region + 1;
    start_region = temp;
end

plot_isoforms_20180131(x_rec,rec_trace,x_5,x_3,data_5,data_3,strand_region,...
    isoform_levels,genome_size,annotation_file,annotation_dir,index,start_region,stop_region)
