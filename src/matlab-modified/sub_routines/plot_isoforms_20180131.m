function plot_isoforms_20180131(x_rec,rec_trace,x_5,x_3,data_5,data_3,...
    strand_region,isoform_levels,genome_size,annotation_file,annotation_dir,index,start_index,stop_index)
...
x_range = start_index:stop_index;
...
if strand_region
    ind_genes = find( (start_f>=min(x_range)) & ...
        (stop_f<=max(x_range)));
else
    min_gene_other = genome_size-max(x_range);
    max_gene_other = genome_size-min(x_range);
...
for i = 1:length(x_5)
    for j = 1:length(x_3)
        ...
        elseif log10(isoform_levels_norm(i,j))>=max_color
            ...
            color_index(i,j) = ceil(n_colors*(log10(isoform_levels_norm(i,j))-min_color)/(max_color-min_color));
        end
    end
end
...
axis off
print(strcat('images/', index), '-dpng')
