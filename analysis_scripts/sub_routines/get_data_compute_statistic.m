function [data,z_peak,z_step,average_data] = ...
    get_data_compute_statistic(files,data_dir,half_width_z_score,half_width_step,...
...
for i = 1:length(files)
    counts = extract_wig(files{i},data_dir,genome_size);
    data(i,:,1) = counts(1,:);
    data(i,:,2) = counts(2,:);
    data(i,:,3) = counts(3,:);
    data(i,:,4) = counts(4,:);
...
for i = 1:length(files)
    for j = 1:4
        ...
        [average_data(i,:,j),z_peak(i,:,j)] = compute_peak_z_score(squeeze(data(i,:,j)),half_width_z_score,...
            average_threshold,filter_type,gap_z);
        z_step(i,:,j) = compute_step_z_score(squeeze(data(i,:,j)),...
